...
public class GoalTrigger : MonoBehaviour
{
    ...
    [SerializeField] private float autoMoveTimeout = 10f; // 安全装置
    ...
    {
        // まだゴール処理中でなく、接触したのが指定したプレイヤーオブジェクトなら
    ...
    private IEnumerator GoalSequence()
    {
        ...
        {
            Debug.LogError("Entrance Point がインスペクタで設定されていません！ 処理を中断します。");
            yield break; // これはUIも表示されない致命的なエラーなのでそのまま
        }
        ...
        while (Vector2.Distance(playerObject.transform.position, entrancePoint.position) > 0.05f)
        {
            ...
            {
                Debug.LogError($"自動移動がタイムアウト ({autoMoveTimeout}秒) しました。演出をスキップしてリザルトを表示します。");
                
                // ★★★ 変更点 ★★★
                // yield break; // コルーチンを停止する (UIが表示されない)
                break;       // while ループだけを脱出する (UI表示処理に進む)
            }

            // MoveTowards で目標地点へ動かす
        ...
        // ★タイムアウトした場合は、ここが "完了" ではなく "スキップ" になる
        Debug.Log("4. 自動移動が完了（またはスキップ）しました。");

        // --- 4. プレイヤーを非表示にする ---
        playerObject.SetActive(false);
        Debug.Log("5. プレイヤーを非表示にしました。");
